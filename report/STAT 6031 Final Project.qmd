...
After taking a closer look at the covariates, we then began our modeling step. We divided the dataset into a training set (80%) and a testing set (20%).
...
By looking at the residuals, we realized that there was room for improvement, so we decided to run several different models selected using stepwise selection, lasso and ridge regression.
...
lm_step <- step(null_model, scope = list(lower = null_model, 
                                         upper = full_model),
...
Once we had examined their in sample performance, we looked at their out-of-sample performance:

```{r}

lm_pred <- predict(simple_lm, data_test)
