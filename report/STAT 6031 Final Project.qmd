...
ggplot(reduced_data, aes(log(price))) +
  geom_histogram(bins = 30, fill = "#5F9DC8", color = "black", alpha = 0.6) +
...
Our models also had similar MSE/MSPE of around 0.16, which indicates that our models performed roughly the same as one another in accurately predicting prices. Additionally, because MSE/MSPE are similar, we see no indications of our model over-fitting the training data:
...
We chose the Lasso model to be our final price prediction model because it had 1) the lowest MSPE of all of our models and 2) was more parsimonious than models of comparable accuracy.
