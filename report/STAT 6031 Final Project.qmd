...
3.  **Variable Selection** -
    ...
    -   After parsing out the Amenities, we found over 2,000 possible indicators. We narrowed this list down to only look at items present in at least 1% of listings. From these \~150 possibilities, we picked 9 features to model with dummy variables ("has_pool", for instance))
...
We first did some **final data cleaning:**
...
-   The dataset was split into a training set (80%) and a testing set (20%) to facilitate model selection and prediction analysis.
...
-   We then did our log-transformation of price, to improve normality of this data:
...
Now that we had our final, cleaned data set, we proceeded to **modeling the price data.** We initially started with a simple linear model.
...
**(Note** : coefficients for Lasso and Ridge regression were located using cross-validation)
...
Our pricing model did a solid job of predicting \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

The model revealed that the most salient factors that determine an Airbnb listing's price in the New Orleans market were:
...
An opportunity for further study exists in further supplementing our data to improve predictions. There were many other potential variables related to location that definitely impact prices - CRIME\*, transit, local housing prices.....Some, if not all, of these likely have an open data source (such as the police department) that would let us explore each of these. if we had more time, we would have liked to follow up on at least some of these factors to assess their impact on pricing.
