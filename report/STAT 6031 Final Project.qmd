...
The distribution plots above highlighted a few concerns with the data that we sought to address:

 1) Our dependent variable (price) is strongly right skewed

2) Most of our covariates are skewed and multimodal

3) A few covariates contain outliers

#### Addressing Non-Normality

Taking a closer look at our dependent variable (price), we decided to log-transform our prices, given their extreme right skewness. As shown below, this step helped make the distribution of values for this variable closer to normality.
...
Since each listing included a URL, we reviewed a sample of these outliers directly on the Airbnb website. For each \`\`outlier," we confirmed that the data was accurate and reflected how it appeared on the platform. Therefore, despite the presence of outliers among our covariates, we chose to retain them as they represent valid and reliable data.
...
To address the impact of highly correlated variables in our model, we created a reduced dataset by removing the most strongly correlated predictors.

In this dataset, we retained **accommodates** and removed **bathrooms**, **bedrooms**, and **beds** because **accommodates** serves as a broader variable that inherently captures aspects such as the number of bedrooms, bathrooms, and beds. Similarly, we kept **review_scores_rating** and excluded the other review categories, as they were highly correlated to the overall score.

Additionally, we retained **availability_365** and removed **availability_30**, **availability_60**, and **availability_90** because our focus is on non-seasonal availability, and we believe **availability_365** is the most representative measure.

Furthermore, we chose to keep **reviews_per_month** instead of **number_of_reviews_ltm**, as it offers a more interpretable metric for activity.

After making these adjustments, the reduced data no longer exhibited significant correlations or multicollinearity among the reamining variables.
...
We created dummy variables for our categorical measures to incorporate them into the model. Additionally, we introduced an interaction term (number_of_reviews \* review_score_rating) to capture the potential non-linear impact of accumulating positive reviews on prices. We believe that the influence of a higher number of positive reviews may have a non-linear impact on prices:
...
To conduct our analysis, we split the dataset into a training set (80%) and a testing set (20%) to facilitate model selection and prediction analysis. Subsequently, we applied a log-transformation to the price variable to enhance he normality of our dependent variable.
...
We initially started with a simple linear model and then explored models using stepwise selection, Lasso, and Ridge regression to evaluate whether any of them could enhance the in-sample fit.

As shown below, the residuals for each model were nearly identical. Therefore, we decided to select our model based on performance metrics, such as in-sample MSE (mean squared error) and out-of-sample MSPE (mean squared prediction error).
...
Below, you can observe the in-sample performance metrics for each model:
...
Upon examining the Mean Squared Prediction Error (MSPE) for each model, we found that all models achieved a similar MSPE of approximately 0.195. This suggests that the models performed comparably in their ability to accurately predict prices.

However, when selecting a model, there is always a trade-off between performance and interpretability. Removing highly correlated variables improves interpretability but may not guarantee better predictive performance. For this reason, we also sought to compare the behavior of different models when using the full set of predictors, including those that are highly correlated.

#### Modeling - Full Dataset (w/ multicollinearity)

To conduct the analysis, we followed the same steps as outlined previously, but this time using the full dataset rather than the reduced dataset. As a result, all the correlated columns that were removed during the EDA step in the reduced dataset were reintroduced into the full dataset model. The results are presented below:
...
After carefully examining all diagnostic plots, as well as both in-sample and out-of-sample performance metrics, we chose the Lasso model to be our final price prediction model because it had the lowest MSPE of all of our models and was more parsimonious than models of comparable accuracy.
...
**Data Supplementation**: Our model could also benefit from incorporating publicly-available data to improve predictions. There were numerous other location-related variables that likely impact prices, such as crime rates, transit availability, and local housing prices. Many of these variables have publicly accessible data source (such as the police department) to could provide this information. If we had more time, we would have liked to follow up on at least some of these factors to assess their impact on pricing.
