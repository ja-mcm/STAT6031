...
From the table above, we observed that all columns pertaining to reviews had the same number of missing values. Upon further inspection, we found that the missing values in these columns corresponded to the same rows. Despite this, we decided to include these rows in the model, as they still provided useful information regarding the other covariates. Additionally, we noted that much of our data is skewed.

To explore this skewness in more detail, we analyzed the distributions using boxplots and density plots:
...
From the distribution plots above, we observed that most of our covariates are skewed and multimodal. And that most of them contain outliers. We took a closer look at which columns contained outliers and how many, and the results are summarized in hte table below:
...
As shown in the table above, many variables contained outliers. However, this was primarily due to some listings that could accommodate 6, 8, 10, or even 20 people, and these were still valid listings. Therefore, although our covariates included outliers, we decided to retain them, as they represented valid data.
...
Since keeping highly correlated variables in a model can negatively impact our results, we have decided to remove some of the highly correlated variables.

We have chosen to keep **accommodates** and remove **bathrooms**, **bedrooms**, and **beds** because **accommodates** is a broader variable that implicitly incorporates aspects such as the number of bedrooms, bathrooms, and beds. Similarly, we have decided to keep **review_scores_rating** and remove the other review categories, as it provides a more general assessment.

We have also decided to keep **availability_365** and remove **availability_30**, **availability_60**, and **availability_90** because we are interested in general availability, and we believe **availability_365** best represents this.

Furthermore, we have chosen to keep **reviews_per_month** instead of **number_of_reviews_ltm**, as it is a more directly interpretable measure of activity.

After removing the variables mentioned earlier from the model, we did not observe any significant correlation or multicollinearity.
...
binary_summary <- report_binary_percentages(reduced_data)
...
Our models also had similar MSPE of around 0.195, which indicates that our models performed roughly the same as one another in accurately predicting prices. Additionally, because MSPE are similar, we see no indications of our model over-fitting the training data:
...
To conduct the analysis, we followed the same steps as previously outlined. The results are presented below:
...
As shown above, when using the model with correlated predictors, all methods demonstrate improved predictive performance. Since the goal of our project is to predict Airbnb prices in the New Orleans area, we have decided to use the model with correlated predictors, as it yields better predictive results.
...
Our pricing model did a solid job of predicting the price of Airbnb listings in New Orleans, with moderately low prediction errors. A noticeable chunk of our residuals were driven by only a few VERY wrong predictions, such as this one:
...
For your "average" Airbnb listing, our model could predict the price reasonably well - for 80% of our test data, we were able to predict the price to within ±\$50. Therefore, we believe we've modeled this system successfully.
...
Nearly all pricing errors in our model were very small - only a few observations were badly misclassified:
