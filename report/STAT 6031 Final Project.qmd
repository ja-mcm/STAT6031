...
ggplot(final_data,aes(price)) + geom_histogram() + labs(title = "Distribution of AirBNB Prices - New Orleans") + theme_bw()
...
ggplot(final_data,aes(log(price))) + geom_histogram() + labs(title = "Distribution of log(AirBNB Prices) - New Orleans") + theme_bw()
...
final_data[,c("neighbourhood_cleansed","latitude", "longitude","amenities_list"):=NULL]
...
data_test[,ACTUAL_PRICE:=exp(log_price)]
data_test[,LASSO_PRED:=exp(predict(lasso_model, x_test,
                               s = lasso_cv$lambda.min))]
data_test[,LASSO_ERROR:=(data_test$log_price - y_hat_test_lasso)^2]
data_test[,PRICE_DIFF:=exp(log_price) - LASSO_PRED]
...
ggplot(data = data_test, aes(x=PRICE_DIFF)) + geom_histogram() + theme_bw()
